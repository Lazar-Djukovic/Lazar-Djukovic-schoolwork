...
for i in range (30,600,30):
  myinvader = Invader(BLUE, 10, 10, i, 30, 15) # invaders are white with size 5 by 5 px
...
for i in range (15,600,30):
  myinvader = Invader(BLUE, 10, 10, i, 45, 15) # invaders are white with size 5 by 5 px
  invader_group.add (myinvader) # adds the new invader to the group of inavders
  all_sprites_group.add (myinvader) # adds it to the group of all Sprites

#Next x
theplayer = Player(RED, 15, 15)
      ...
      if event.key == pygame.K_LEFT: # - if the left key pressed
       ...
       theplayer.player_set_speed(3) # speed set to 3
        ...
        if theplayer.bullet_count > 0:
          thebullet = Bullet(YELLOW, 7, theplayer.rect.x, theplayer.rect.y)
          bullet_group.add(thebullet)
